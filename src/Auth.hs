...
module Auth (generateToken, makePayload, decodeToken, getValidUser, userId, userName, Payload(..)) where
...
makePayload :: (Int64, String) -> Int64 -> IO Payload
makePayload (id, name) offset = Payload id name <$> timeOffset offset
...
getValidUser (Right src@(Payload _ _ expired)) = do
    time <- now
